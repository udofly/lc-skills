...
3. **制定测试策略**
   ...
   | 纯函数（格式化、映射、校验） | 正常值、边界值、异常值 | 高 |
   | Vue 组件（.vue） | 不测试（见测试范围规则） | — |
   | 请求层（MwpXxx.ts） | 不测试（简单包装，无逻辑） | — |
   | 路由配置 | 不测试 | — |
   | 样式文件 | 不测试 | — |
...
4. **生成测试文件**
   ...
   import { formatPrice, getStatusLabel, validateCouponForm } from '../utils';
   ...
   describe('getStatusLabel', () => {
       it('should return correct label for each status', () => {
           expect(getStatusLabel(1)).toBe('进行中');
           expect(getStatusLabel(2)).toBe('已结束');
       });

       it('should return default for unknown status', () => {
           expect(getStatusLabel(99)).toBe('未知');
       });
   });
   ```
   ...
   - **测试纯输入→输出关系**，覆盖正常值、边界值、异常值
   - **必要时 mock 外部依赖**（如被测函数内部调用了 API），但优先测试无副作用的纯函数
