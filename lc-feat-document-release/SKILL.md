...
2. **【核心】增量更新项目功能文档**
   ...
   **回退**：若项目无此类技能、或该技能未声明「文档维护约定」→ 退化为通用结构 `docs/features/{module}.md`，并提示用户在项目技能里补充「文档维护约定」以获得更好的结构化文档。
   ...
   从 `docs/pipeline/{feat-name}/spec.md`（旧版功能：`requirement.md`）或改动文件路径中提取模块名：
   ...
   **B. 回退项目**（无项目约定，2.1 退化）：写到 `docs/features/{module}.md`，用下方通用模板：
   ...
   - 从 spec.md「一、需求」（旧版：`requirement.md`）提取：用户操作流程、业务规则
   - 从 spec.md「二、设计」（旧版：`design.md`）提取：涉及组件、接口依赖
   ...
   ### 2.6 打戳与记账（若项目声明了「更新账本」）

   项目「文档维护约定」若声明了更新账本（如 `updates.md`）与文档时间戳约定，则同步执行：

   - **打戳**：给本次新建/修改的每篇文档 frontmatter 写入/更新 `updated: YYYY-MM-DD` 与 `commit: <短hash>`（取已合并 PR 的 merge commit；独立模式取当前 HEAD）。
   - **记账**：向账本「更新记录」表**最上方**追加一行：日期 / 同步范围（本功能 commit 区间）/ 来源（`lc-feat:document-release {feat-name}`）/ 更新文档清单 / 摘要。
   - **不推进**账本 frontmatter 的 `last_sync` 锚点——那是 `lc-doc-sync` 的职责（本 skill 只覆盖本功能的文档，同期可能还有其它未记账变更）。
...
7. **漏网变更检查（`lc-doc-sync --check`）**

   若安装了 `lc-doc-sync` 且项目声明了「更新账本」：按其 `--check` 模式跑一次**只读**检查（账本锚点 → diff 到 HEAD → 映射过时文档）。目的：发布收尾时零成本发现**同期绕过流水线的变更**（hotfix、重构、他人合并的功能）还有哪些没进文档。

   - 只报告，不修改任何文件、不推进锚点；发现漏网 → 在总结中列出清单并建议用户跑 `/lc-doc-sync` 补齐。
   - 未安装 `lc-doc-sync` 或项目无账本 → 跳过本步。

8. **分支清理提醒 + 输出总结**

   输出：
   - 文档更新情况（更新了哪个模块文档、追加了什么内容）
   - 漏网变更检查结果（过时文档清单，或"无漏网"）
   - Mock 标记情况
   - 分支清理建议（不自动执行）
...
- Mock 标记只提醒不替换
- 漏网变更检查（步骤 7）只报告不顺手同步；补齐必须由用户显式跑 `/lc-doc-sync`
- 分支清理只提醒不执行
