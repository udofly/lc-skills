...
2. **Preamble — 每步执行前的上下文注入**
   ...
   | verify | requirement.md + design.md 摘要 + implement 改动文件列表 |
...
3. **按顺序执行各步骤**
   ...
   - 执行 `/lc-feat:design` 逻辑（已包含设计自审，生成后自动审查并修订）
   - 产出：`design.md`（含自审修订记录）
   - **质量门控**：如果自审发现 P1 级严重问题，使用 **AskUserQuestion** 提示用户确认
   - **暂停**：`是否继续 Step 3（编码实现）？ y — 继续 / s — 跳过`

   ### Step 3: 编码实现
   - **前置检查**：
     - full 模式：`design.md` 必须存在
