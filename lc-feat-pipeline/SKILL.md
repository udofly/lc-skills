...
description: 功能开发总控技能。串联需求分析→设计(含自审)→编码→验证→E2E的完整流水线（不含CR/PR），支持从任意步骤开始和断点恢复。CR/PR请单独使用 /lc-feat:pr。当用户说"开始流水线"、"完整开发"、"走流程"、"pipeline"时触发。
license: MIT
metadata:
...
**需求与设计合并产出一份 `spec.md`（需求与设计文档）**：requirement 步骤写「基本信息 + 一、需求」，design 步骤在同一文件追加「二、设计 + 自审修订」。full 模式下这两步**连续执行、中间不暂停**，design 完成后展示需求+设计合并摘要，**只审核一次**。
...
1. **初始化流水线**
   ...
   **progress.yaml 格式**：
   ...
   steps:
     requirement: { status: pending, file: spec.md }
     design: { status: pending, file: spec.md, depends: [requirement] }
     implement: { status: pending, file: "", depends: [design] }
     verify: { status: pending, file: verify-report.md, depends: [implement] }
...
2. **Preamble — 每步执行前的上下文注入**
   ...
   | 当前步骤 | 需要加载的前序产出 |
   |---------|-------------------|
   | design | spec.md「一、需求」的"需求背景"和"功能描述" |
   | implement | spec.md「二、设计」的"改动文件清单"和"页面组件设计" |
   | verify | spec.md 需求+设计摘要 + implement 改动文件列表 |
   | e2e | spec.md 需求部分的用户操作流程 + 设计章节的路由和页面结构 + implement 改动文件列表 |
...
3. **按顺序执行各步骤**
   ...
   - **前置检查**：`spec.md` 的「一、需求」部分必须存在
   - **Preamble**：加载 spec.md 需求部分摘要 + 相关 memory
   - 执行 `/lc-feat:design` 逻辑（在 spec.md 追加「二、设计」章节，已包含设计自审，生成后自动审查并修订）
   - 产出：`spec.md`（追加设计章节 + 自审修订记录）
   - **质量门控**：如果自审发现 P1 级严重问题，使用 **AskUserQuestion** 提示用户确认
   - **暂停（需求+设计合并审核，全流水线第一次暂停）**：展示需求要点摘要 + 设计摘要 + 自审修订记录，然后：`是否继续 Step 3（编码实现）？ y — 继续 / s — 跳过`
     - 用户如对需求理解有异议：修订 spec.md 需求部分并同步重跑受影响的设计章节，记录到 decisions，再次展示确认

   ### Step 3: 编码实现
   - **前置检查**：
     - full 模式：`spec.md` 必须存在且包含「二、设计」章节
     - lite 模式：`spec.md` 的需求部分必须存在
   - **Preamble**：加载 spec.md 设计章节的改动文件清单和页面组件设计 + decisions + 相关 memory
   ...
   - **Preamble**：加载 spec.md 需求+设计摘要 + 改动文件列表
   - 执行 `/lc-feat:verify` 逻辑（三阶段：静态检查 → 单元测试 → 综合质量分析）
   - 产出：`verify-report.md`
   - **质量门控**：
     - Lint 0 error + 测试通过 + QA >= 8 分 → 自动通过
     - 有阻塞问题 → **暂停**：
       > "代码验证发现 {N} 个问题需处理（详见 verify-report.md）。"
   ...
   - **Preamble**：加载 spec.md 需求部分的用户操作流程 + 设计章节的路由和页面结构 + 改动文件列表
...
4. **自动断点恢复**
   ...
   - **`design` 步骤特殊判定**：`spec.md` 存在**且包含「二、设计」章节**才算完成（requirement 和 design 共用一个文件，仅文件存在不够）
   - **旧版兼容**：历史功能目录下若是 `requirement.md` / `design.md` 旧布局，按旧文件判定并沿用，不迁移为 spec.md
...
AI 将需求写入 `spec.md` 后**不暂停**，继续在同一文件追加「二、设计」并完成自审，然后一次性展示需求要点 + 设计摘要 + 自审修订记录，等待确认。此后每步输入 `y` 即可推进。
...
```
AI：流水线已初始化：fix-price-display（lite 模式，3 步）
...
```bash
用户：/lc-feat:pipeline 3.38-coupon --from=verify
```

```
AI：流水线从 verify 步骤开始：3.38-coupon
    ⏭️ requirement — 跳过
    ⏭️ design — 跳过
    ⏭️ implement — 跳过
    🔄 verify — 从此开始
...
AI：🏁 流水线全自动完成：商品标签筛选
    ...
    ✅ design — done（3 个文件改动，自审修订 1 个问题）
    ✅ implement — done（新增 TagFilter.vue，修改 GoodsSearch.vue）
    ✅ verify — done（Lint 2 个自动修复，Test 5/5 通过，QA 8.2/10）
...
AI：Step 5（E2E 测试）已跳过，标记为 skipped。
    流水线完成。如需 Code Review 和 PR 创建，请使用 /lc-feat:pr
```
