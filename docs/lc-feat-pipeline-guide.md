...
```
  ...
  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
  │ 8. 创建 PR   │ ←  │ 7. QA 分析   │ ←  │ 6. 编写测试   │ ←  │ 5. 代码检查   │
  │ pr            │    │ qa           │    │ test          │    │ lint          │
  │ + code review │    │ 五维度 0-10  │    │ Vitest        │    │ ESLint/TS     │
  └──────────────┘    └──────────────┘    └──────────────┘    └──────────────┘
  ...
  ┌──────────────────────────────────────────────────┐
  │ pipeline — 一键串联 1→8，支持 --from 断点恢复     │
  └──────────────────────────────────────────────────┘
```
...
一条命令启动完整流水线。AI 按顺序执行 8 个步骤，每步完成后暂停等你确认：
- 回复 **"继续"** → 进入下一步
...
/lc-feat:test goods-manage            # 第6步：编写测试
/lc-feat:qa goods-manage              # 第7步：QA 分析
/lc-feat:pr goods-manage              # 第8步：创建 PR
```
...
可用的 step 值：`requirement`、`design`、`design-review`、`implement`、`lint`、`test`、`qa`、`pr`
...
| **产出** | PR URL + `docs/pipeline/{feat}/pr-review.md` |

**执行内容：**
1. QA 评分门控检查（< 8 分会警告）
2. 对 diff 进行代码 Review（命名、规范、风险评估）
3. 创建功能分支 `feat/{feat-name}`
4. 提交代码（Angular commit 规范）
5. 创建 PR（包含需求背景、变更清单、测试方案、QA 评分）
...
| 项目 | 说明 |
|------|------|
| **命令** | `/lc-feat:pipeline <feat-name> [--from=<step>]` |
...
/lc-feat:pipeline xxx                  # 走完 8 步流水线
/ship                                  # 合并 PR + CI
...
### 确认机制
- 每步完成后暂停等待确认
- "继续" → 下一步 | 修改意见 → 当前步骤修改 | "跳过" → 跳过 | "回退到{步骤}" → 返回
