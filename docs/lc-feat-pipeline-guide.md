...
> 版本: 2.2 | 更新日期: 2026-07-09 | 全局 skill，适用于所有项目
...
- [3. 快速开始](#3-快速开始)
  ...
  - [3.2 全自动模式](#32-全自动模式)
  - [3.3 逐步执行](#33-逐步执行)
  - [3.4 断点恢复与功能看板](#34-断点恢复与功能看板)
  - [3.5 多功能与昼夜工作流](#35-多功能与昼夜工作流)
...
- **流水线按分支归档，一流水线多功能** — 分支目录 `docs/pipeline/{branch-name}/` 下可有多条流水线（每条一个子文件夹），流水线内含多个功能的独立「需求+设计」spec（编号自动 +1），verify/e2e 报告汇总一份、按功能分章节
- **昼夜工作流** — 白天 `--until=design` 批量出设计（人工审核），晚上 `--start` 串行批量跑实现+验证，早上按功能分组 commit
- **逐步确认** — 每步完成后暂停，`y` 继续 / `s` 跳过（需求分析→技术设计连续执行、合并审核一次）
- **单文档产出** — 每个功能的需求与设计合并为一份 `spec-{n}-{feat}.md`（一、需求 + 二、设计 + 自审修订），重复内容只写一次
- **全自动模式** — `--auto` 参数一口气跑完，无需确认，适合小功能快速开发
- **断点恢复** — 流水线中断后可从任意步骤继续；多功能时展示看板选择
...
**数据流：** 每步的产出文件是下一步的输入，全部存储在 `docs/pipeline/{branch-name}/{pipeline-name}/` 下（分支目录 → 流水线子目录，一个分支可有多条流水线，流水线内含多个功能的 spec 与汇总报告）。
> 为简洁起见，本文后续出现的 `docs/pipeline/{pipeline-name}/` 均指 `docs/pipeline/{branch-name}/{pipeline-name}/` 两级结构。
...
流水线中断后，直接重新进入即可**自动断点恢复**：读取 `progress.yaml` 确定各功能各步状态，校验产出文件后从第一个未完成步骤继续，并展示进度摘要和历史决策。

- 只有一个未完成功能 → 自动定位该功能继续
- 多个未完成功能 → 展示**功能看板**，选择继续哪个功能（编号 / 功能名 / "新增"）

```bash
/lc-feat:pipeline                                  # 自动检测进度；多功能时展示看板
/lc-feat:pipeline --feat=2                         # 定位功能 #2，从其断点继续
/lc-feat:pipeline --feat=2 --from=verify           # 定位功能 #2，显式从验证开始
```
...
**核心模型：流水线按分支归档（`docs/pipeline/{branch-name}/{pipeline-name}/`），一个分支下可有多条流水线，一条流水线多个功能。**

- 分支目录名：分支名去 `feature/` 前缀；流水线名：用户显式指定优先，否则默认同分支目录名
- 同流水线新增需求 → 功能编号自动 +1，各建一份 `spec-{n}-{feat}.md`
- verify-report.md / e2e-report.md 各一份，按功能分章节，更新时只覆盖本功能章节
- **写文档阶段（requirement/design）多功能可并行；写代码阶段（implement 及之后）同一时刻只允许一个功能进行**
- design 完成时自动做**跨功能文件冲突预检**，重叠文件标出执行顺序并记入 decisions
- implement 完成回写 `files_changed`，作为功能间的代码边界与 commit 分组依据

**昼夜工作流（推荐）：**

```bash
# 白天：逐个出需求+设计（人工审核，审核通过标记 design_approved）
/lc-feat:pipeline --until=design 优惠券列表页，搜索+表格+新增编辑弹窗
/lc-feat:pipeline --until=design 优惠券导出功能，列表页加导出按钮

# 晚上：串行批量执行所有已审核设计的功能（implement → verify → e2e，--auto 门控）
/lc-feat:pipeline --start

# 早上：按 start-report.md 的 files_changed 分组，逐功能 commit
```

批量执行规则（--start）：只执行 `design_approved: true` 的功能；严格串行；单个功能失败标记 `blocked` 并继续下一个，不阻塞整批；产出 `start-report.md`（每功能状态 + files_changed 分组 + 建议 commit message）。
...
| **输入** | 文字描述、截图路径、外部文档 |
| **产出** | `docs/pipeline/{pipeline}/spec-{n}-{feat}.md` 的「基本信息 + 一、需求」部分 |
...
| **前置** | 本功能 `spec-{n}-{feat}.md` 的需求部分已存在 |
| **产出** | 同一 spec 文件追加「二、设计」章节 + 自审修订记录（不生成独立 design.md）；流水线内多功能时另做跨功能文件冲突预检 |
...
| **产出** | `docs/pipeline/{pipeline}/verify-report.md` 中本功能的章节（含 Lint、Test、QA，章节化更新） |
| **下一步** | `/lc-feat:e2e` |
...
| **产出** | 测试文件 `tests/e2e/specs/{feat}.spec.ts` + 截图 + `docs/pipeline/{pipeline}/e2e-report.md` 中本功能的章节 |
...
| 项目 | 说明 |
|------|------|
| **命令** | `/lc-feat:pipeline [pipeline-name] [--feat=<id\|name>] [--from=<step>] [--until=<step>] [--mode=full\|lite] [--auto] [--start]` |
| **产出** | `docs/pipeline/{pipeline-name}/progress.yaml`（features 数组）+ 全部产物 |

**总控负责：**
- 按顺序串联 5 个主流程步骤（不含 CR/PR）
- **一个分支一个流水线文件夹**：pipeline-name 用户指定优先，否则取分支名；同流水线新增功能编号自动 +1
- 维护 `progress.yaml` 进度文件（features 数组：每功能独立 steps/mode/design_approved/files_changed + decisions 带 feature 字段）
- 每步执行前运行 **Preamble** 注入上下文：进度状态、同流水线其他功能概览、历史决策、相关 Memory（最多 3 条）、前序产出摘要
- 每步暂停提示 `y — 继续 / s — 跳过`（requirement → design 连续执行不暂停，design 后合并审核一次，审核通过标记 `design_approved`）
- design 完成时做跨功能文件冲突预检；implement 完成回写 files_changed；写代码阶段功能间互斥
- `progress.yaml` 已存在时自动断点恢复；多功能时展示看板；`--feat` 定位功能，`--from` 显式指定起点，`--until` 执行到指定步骤后结束（白天只出设计用 `--until=design`）
- 支持 `--mode=full|lite`（默认按需求类型自动选择，按功能独立记录）、`--auto` 单功能全自动、`--start` 串行批量执行已审核功能（产出 start-report.md）
- QA 评分门控（< 8 分警告，< 6 分建议回退）
- 每个功能全部完成后写入 Memory（project/feedback 类型，按功能粒度，中途中断不写入）
- 旧版单功能目录（无 features 数组的 progress.yaml）按旧逻辑沿用，不迁移

---

## 6. 产出文件总览

一个分支的所有产物集中存储在分支目录 `docs/pipeline/{branch-name}/` 下，每条流水线一个子文件夹：

```
docs/pipeline/{branch-name}/{pipeline-name}/
├── progress.yaml           # 本流水线总进度（features 数组，管所有功能 + decisions 决策记录）
├── spec-1-{feat-a}.md      # 功能 #1 的需求与设计（基本信息 + 一、需求 + 二、设计 + 自审修订）
├── spec-2-{feat-b}.md      # 功能 #2 的需求与设计（编号自动 +1）
├── verify-report.md        # 代码验证报告，汇总一份，按功能分章节（Lint / Test / QA + 0-10 评分）
├── e2e-report.md           # E2E 测试报告，汇总一份，按功能分章节
├── start-report.md         # 批量执行总结（--start 产出：每功能状态 + commit 分组建议）
├── pr-review.md            # PR 代码审查报告
├── document-release.md     # 发布后总结（document-release 后生成）
└── api-sync-report.md      # 接口同步报告（api-sync 后生成）
```

> 旧版布局（`docs/pipeline/` 一级直建的流水线目录，或单功能目录 + `spec.md`）的历史流水线按旧结构沿用，不迁移；新建流水线一律使用「分支目录/流水线目录」两级结构。
...
| 命令 | 说明 | 触发词 |
|------|------|--------|
| `/lc-feat:pipeline [name]` | 一键流水线（多功能看板/断点恢复） | "开始流水线"、"走流程"、"pipeline" |
| `/lc-feat:pipeline --until=design <描述>` | 白天只出需求+设计（审核后就绪） | "只出设计"、"先出设计" |
| `/lc-feat:pipeline --start` | 串行批量执行已审核功能 | "批量执行"、"夜跑"、"开始跑" |
| `/lc-feat:pipeline --feat=<n>` | 定位流水线内某个功能 | — |
...
### 命名规范
- **branch-name（分支目录名）**：分支名去掉 `feature/` 前缀（如 `feature/3.38-coupon` → `3.38-coupon`）；**该分支所有流水线都归档在 `docs/pipeline/{branch-name}/` 下**
- **pipeline-name（流水线子文件夹名）**：用户显式指定优先；未指定时默认与 branch-name 相同
- **feat-name（功能名）**：流水线内每个功能一个，kebab-case（如 `coupon-list`）；spec 文件命名 `spec-{n}-{feat-name}.md`，编号自动 +1
- 名称用于：产物目录 `docs/pipeline/{branch-name}/{pipeline-name}/`、spec 文件名、汇总报告章节标题、PR 标题
...
**Q: 同一条流水线可以重复进入吗？**
A: 可以。`progress.yaml` 已存在时自动进入断点恢复：单个未完成功能直接继续；多个功能展示看板选择。重跑某功能某步用 `--feat=<n> --from=<step>`。

**Q: 同一个分支有多个功能怎么办？**
A: 这正是推荐用法：一个分支一个流水线文件夹，每个功能一份 `spec-{n}-{feat}.md`（新增需求编号自动 +1），verify/e2e 报告按功能分章节汇总。需求/设计阶段可多功能并行，编码及之后阶段同一时刻只跑一个功能。白天 `--until=design` 批量出设计，晚上 `--start` 串行批量执行，早上按 start-report.md 分组逐功能 commit。

**Q: 只能用于 Vue 项目吗？**
A: skill 本身通用。implement 会参考项目的 CLAUDE.md 和现有代码来生成代码，适用于任何前端项目。

**Q: 如何查看流水线进度？**
A: 直接 `/lc-feat:pipeline` 查看功能看板，或查看 `docs/pipeline/{pipeline-name}/progress.yaml`（features 数组：每功能步骤状态 + 历史决策）。
...
>
> 注：本示例为**单功能流水线**场景，交互流程在新版多功能结构下完全一致，仅文件名不同——`spec.md` 对应新版的 `spec-1-coupon-manage.md`，progress.yaml 为 features 数组格式。多功能与昼夜工作流示例见 [3.5 节](#35-多功能与昼夜工作流)。
...
AI（当前在 `feature/coupon-manage` 分支）创建 `docs/pipeline/coupon-manage/coupon-manage/` 目录（分支目录/流水线目录），初始化 `progress.yaml`，进入第一步。
...
AI：**创建 PR** 完成！
    ...
    - Review 报告: docs/pipeline/coupon-manage/coupon-manage/pr-review.md
    - 全部产物在 docs/pipeline/coupon-manage/coupon-manage/ 下
```
...
AI：发布后处理完成：
    ...
    - 总结: docs/pipeline/coupon-manage/coupon-manage/document-release.md
```
...
AI：接口同步完成：
    ...
    - TypeScript 类型已更新
    - 类型检查通过
    - 报告: docs/pipeline/coupon-manage/coupon-manage/api-sync-report.md
```
...
```
docs/pipeline/coupon-manage/coupon-manage/
