...
> 版本: 1.3 | 更新日期: 2026-05-25 | 全局 skill，适用于所有项目
...
- [3. 快速开始](#3-快速开始)
  ...
  - [3.3 逐步执行](#33-逐步执行)
  - [3.4 断点恢复](#34-断点恢复)
- [4. 主流程 Skill 详解（8步）](#4-主流程-skill-详解8步)
  ...
  - [4.7 E2E 端到端测试 e2e](#47-e2e-端到端测试-e2e)
  - [4.8 QA 分析 qa](#48-qa-分析-qa)
- [4.9 CR / 创建 PR（独立步骤）](#49-cr--创建-pr独立步骤不在-pipeline-中执行)
...
`lc-feat` 是一套 **端到端的功能开发流水线**，由 12 个 Claude Code skill 组成，覆盖从需求分析到 PR 发布的完整生命周期。
...
```
  ...
  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
  │ 8. QA 分析   │ ←  │ 7. E2E 测试  │ ←  │ 6. 编写测试   │ ←  │ 5. 代码检查   │
  │ qa           │    │ e2e           │    │ test          │    │ lint          │
  │ 五维度 0-10  │    │ Playwright    │    │ Vitest        │    │ ESLint/TS     │
  └──────────────┘    └──────────────┘    └──────────────┘    └──────────────┘
...
一条命令启动完整流水线。AI 按顺序执行 8 个步骤（不含 CR/PR），每步完成后暂停提示：
- 回复 **`y`** → 继续执行下一步
- 回复 **`s`** → 跳过当前步骤
...
/lc-feat:e2e goods-manage             # 第7步：E2E 端到端测试
/lc-feat:qa goods-manage              # 第8步：QA 分析
...
### 4.9 CR / 创建 PR（独立步骤，不在 pipeline 中执行）
...
```
AI：**[7/8 E2E 测试]** 已完成。4 个用例通过，12 张截图。
    回复"继续"进行 QA 分析。
```
...
```
AI：**[8/9 QA 分析]** 已完成。评分 8.6/10，达到发布标准。
    回复"继续"创建 PR。
```
...
### Step 11：PR 合并后收尾（document-release）
