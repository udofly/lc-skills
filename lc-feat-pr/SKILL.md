...
1. **加载上下文**
   ...
   读取：
   - `docs/pipeline/{feat-name}/spec.md`（需求与设计文档；旧版功能：`requirement.md` + `design.md`）
...
5. **创建成功后提示文档同步交接**（不自动执行）：

   > "PR 已创建。合并后请运行 `/lc-feat:document-release <feat-name>` 同步项目文档。"

   该步骤让"代码改动 → 文档更新"形成闭环，避免文档随迭代过时；`document-release` 会按项目 skill 的「文档维护约定」更新对应模块页面文档。
